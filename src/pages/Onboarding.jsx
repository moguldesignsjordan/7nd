...
export default function Onboarding() {
  ...
  async function handleFinish() {
    ...
    try {
      const profile = {
        name: data.name,
        goal: data.goal,
        sleepWindow: { wake: data.wake, bed: data.bed },
  ...
  const steps = [
    ...
    <StepReady      data={data} saving={saving} onFinish={handleFinish} />,
  ]

  return (
    <div style={{ minHeight: '100vh', background: 'var(--color-linen)', display: 'flex', flexDirection: 'column' }}>
      {step > 0 && step < 4 && (
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            {[1,2,3].map(i => (
              <div key={i} style={{
                ...
                background: i <= step ? 'var(--color-living-green)' : 'var(--color-warm-stone)',
  ...
  return (
    <div style={{ padding: '80px 28px 40px', display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
      <div style={{ flex: 1 }}>
        ...
        <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 36, fontWeight: 400, color: 'var(--color-forest-deep)', lineHeight: 1.15, marginBottom: 16 }}>
          Align with<br /><em>your nature</em>
        </h1>
        <p style={{ fontSize: 15, color: 'var(--color-driftwood)', lineHeight: 1.7, fontFamily: 'var(--font-sans)', marginBottom: 36 }}>
          Your health on any given day is a function of your environment — not just your choices.
        </p>
        <label style={{ display: 'block', marginBottom: 6 }}>
          <span className="text-label">Your first name</span>
        </label>
        <input type="text" placeholder="e.g. Jordan" value={data.name}
      ...
      <PrimaryButton label="Begin →" disabled={!data.name.trim()} onClick={onNext} />
    </div>
  )
}
  ...
  return (
    <div style={{ padding: '40px 24px', display: 'flex', flexDirection: 'column', minHeight: '85vh' }}>
      ...
      <div style={{ flex: 1, paddingTop: 16 }}>
        ...
        <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 28, fontWeight: 400, color: 'var(--color-forest-deep)', lineHeight: 1.2, marginBottom: 8 }}>
          What's your primary<br />health goal?
        </h2>
        <p style={{ fontSize: 13, color: 'var(--color-driftwood)', marginBottom: 24, lineHeight: 1.6, fontFamily: 'var(--font-sans)' }}>
          This shapes your coaching and readiness priorities.
        </p>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {GOALS.map(g => (
            <button key={g.id} onClick={() => set('goal', g.id)} style={{
              ...
              <div>
                <p style={{ fontSize: 14, fontWeight: 500, fontFamily: 'var(--font-sans)', color: 'var(--color-bark)', marginBottom: 2 }}>{g.label}</p>
                <p style={{ fontSize: 12, color: 'var(--color-driftwood)', fontFamily: 'var(--font-sans)' }}>{g.desc}</p>
              </div>
              {data.goal === g.id && <span style={{ marginLeft: 'auto', color: 'var(--color-living-green)', fontSize: 18 }}>✓</span>}
            </button>
          ))}
        </div>
      </div>
  ...
  return (
    <div style={{ padding: '40px 24px', display: 'flex', flexDirection: 'column', minHeight: '85vh' }}>
      ...
      <div style={{ flex: 1, paddingTop: 16 }}>
        ...
        <p style={{ fontSize: 13, color: 'var(--color-driftwood)', marginBottom: 24, lineHeight: 1.6, fontFamily: 'var(--font-sans)' }}>
          Consistency matters more than duration. Your wake time anchors your entire circadian rhythm.
        </p>
        <div style={{ background: 'var(--color-meadow-mist)', border: '1px solid var(--color-sage-glow)', borderRadius: 12, padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
          <span style={{ fontSize: 13, color: 'var(--color-forest-deep)', fontFamily: 'var(--font-sans)' }}>😴 Estimated sleep</span>
          <span style={{ fontFamily: 'var(--font-display)', fontSize: 18, color: 'var(--color-living-green)', fontWeight: 600 }}>{calcSleep(data.bed, data.wake)}</span>
        </div>
        <div style={{ display: 'flex', gap: 14 }}>
          <div style={{ flex: 1 }}>
            <p className="text-label" style={{ marginBottom: 8 }}>Bedtime</p>
            <select value={data.bed} onChange={e => set('bed', e.target.value)} style={selectStyle}>
              {BED_TIMES.map(t => <option key={t}>{t}</option>)}
            </select>
          </div>
          <div style={{ flex: 1 }}>
            <p className="text-label" style={{ marginBottom: 8 }}>Wake time</p>
            <select value={data.wake} onChange={e => set('wake', e.target.value)} style={selectStyle}>
  ...
  return (
    <div style={{ padding: '40px 24px', display: 'flex', flexDirection: 'column', minHeight: '85vh' }}>
      ...
      <div style={{ flex: 1, paddingTop: 16 }}>
        ...
        <p style={{ fontSize: 13, color: 'var(--color-driftwood)', marginBottom: 24, lineHeight: 1.6, fontFamily: 'var(--font-sans)' }}>
          Your UV index, air quality, and daily plan adapt to your environment.
        </p>
        <p className="text-label" style={{ marginBottom: 8 }}>City</p>
        <input type="text" placeholder="e.g. Miami, FL" value={data.location}
          onChange={e => set('location', e.target.value)}
          style={{ ...inputStyle(false), marginBottom: 24 }}
          onFocus={e => e.target.style.borderColor = 'var(--color-living-green)'}
          onBlur={e => e.target.style.borderColor = 'var(--color-warm-stone)'}
        />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          {CLIMATES.map(c => (
            <button key={c.id} onClick={() => set('climate', c.id)} style={{
              ...
              <span style={{ fontSize: 12, fontWeight: 500, fontFamily: 'var(--font-sans)', color: 'var(--color-bark)' }}>{c.label}</span>
            </button>
          ))}
        </div>
      </div>
      <PrimaryButton label="Build my plan →" disabled={!data.location.trim() || !data.climate} onClick={onNext} />
    </div>
  )
}

function StepReady({ data, saving, onFinish }) {
  return (
    <div style={{ minHeight: '100vh', background: 'var(--color-forest-deep)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '40px 28px', textAlign: 'center' }}>
      <span style={{ fontSize: 60, marginBottom: 24 }}>🌅</span>
      <p className="text-label" style={{ color: 'var(--color-sage-glow)', marginBottom: 12 }}>You're all set</p>
      <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 32, fontWeight: 400, color: 'var(--color-spring-leaf)', lineHeight: 1.2, marginBottom: 16 }}>
        Welcome, <em>{data.name}</em>
      </h1>
      <p style={{ fontSize: 14, color: 'var(--color-sage-glow)', lineHeight: 1.7, maxWidth: 280, marginBottom: 40, fontFamily: 'var(--font-sans)' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, justifyContent: 'center', marginBottom: 48 }}>
        {[
          ...
          <span key={pill} style={{ padding: '6px 14px', background: 'rgba(168,188,114,0.15)', border: '1px solid rgba(168,188,114,0.3)', borderRadius: 20, fontSize: 12, color: 'var(--color-spring-leaf)', fontFamily: 'var(--font-sans)' }}>{pill}</span>
        ))}
      </div>
      <button onClick={onFinish} disabled={saving} style={{
        width: '100%', maxWidth: 320, padding: '16px 24px',
        background: saving ? 'rgba(168,188,114,0.3)' : 'var(--color-living-green)',
...
function BackButton({ onClick }) {
  return (
    <button onClick={onClick} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-driftwood)', fontSize: 13, fontFamily: 'var(--font-sans)', padding: '0 0 4px' }}>
      ← Back
    </button>
  )
}
...
const inputStyle = () => ({
  ...
  color: 'var(--color-bark)', outline: 'none',
})
...
const selectStyle = {
  ...
  fontFamily: 'var(--font-sans)',
  color: 'var(--color-bark)', outline: 'none', cursor: 'pointer',
}
